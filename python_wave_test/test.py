...
def main():
	...
	#pos1 = np.random.uniform((-2,0),(-1,1),(int(N/2),2))
	#pos2 = np.random.uniform((1,0),(2,1),(int(N/2),2))

	#pos = np.concatenate((pos1,pos2))
	pos = np.random.uniform((-2,0),(-1.5,1.5),(N,2))
	...
	for i in range(Nt):
		...
		vel[:,1] = np.where(pos[:,1] < 0, -vel[:,1]*bounce_factor, vel[:,1])
		pos[:,1] = np.where(pos[:,1] < 0, -pos[:,1], pos[:,1])
		vel[:,0] = np.where(pos[:,0] < left_x, -vel[:,0]*bounce_factor, vel[:,0])
		pos[:,0] = np.where(pos[:,0] < left_x, left_x+(left_x-pos[:,0]), pos[:,0])
		vel[:,0] = np.where(pos[:,0] > right_x, -vel[:,0]*bounce_factor, vel[:,0])
		pos[:,0] = np.where(pos[:,0] > right_x, right_x+(right_x-pos[:,0]), pos[:,0])
